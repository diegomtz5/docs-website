...
This page documents the metrics collected when monitoring Apache Kafka with OpenTelemetry. Metrics are collected via the [Kafka metrics receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/kafkametricsreceiver), [JMX receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/jmxreceiver), and [OpenTelemetry Java agent](https://opentelemetry.io/docs/languages/java/automatic/) for client-side metrics.
...
* **[JMX receiver](https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/receiver/jmxreceiver)**: Connects to JMX port (typically 9999) to collect detailed broker metrics and JVM metrics
...
## JMX receiver metrics [#jmx-metrics]
...
1. **Default Kafka target system** - Built-in Kafka-specific metrics from `target_system: kafka` ([kafka.yaml](https://github.com/open-telemetry/opentelemetry-java-contrib/blob/main/jmx-scraper/src/main/resources/kafka.yaml))
...
These additional Kafka metrics and JVM metrics are collected when using a custom JMX configuration file (as shown in the [self-hosted setup](/docs/opentelemetry/integrations/kafka/self-hosted#jmx-config)). The custom configuration allows you to collect additional Kafka-specific metrics beyond the default set, as well as JVM health metrics.

<CollapserGroup>
  <Collapser
    id="jmx-cluster-level-metrics"
    title="Cluster-level metrics"
  >
...
<table>
  ...
  <Collapser
    id="jmx-broker-specific-metrics"
    title="Broker-specific metrics"
  >
...
## Querying metrics in New Relic [#query-metrics]

Use NRQL to query your Kafka metrics. All queries should include the `instrumentation.provider='opentelemetry'` filter to ensure you're querying OpenTelemetry data.
...
## Alerting on Kafka metrics [#alerting]
...
* **[Create NRQL alert conditions](/docs/alerts/create-alert/create-alert-condition/create-nrql-alert-conditions/)** to monitor your Kafka clusters
* **[Use pre-built dashboard](/docs/opentelemetry/integrations/kafka/overview/#4-pre-built-dashboards)** or **[create custom ones](/docs/new-relic-solutions/build-nr-ui/contribute-nr-io/create-a-dashboard/)** to visualize Kafka performance
* **[Query metric data types](/docs/data-apis/understand-data/metric-data/query-metric-data-type/)** to learn advanced techniques for working with OpenTelemetry metrics
